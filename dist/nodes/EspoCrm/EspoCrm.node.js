...
async function espoRequestBinary(method, endpoint, options = {}) {
    const credentials = (await this.getCredentials('espoCrmApi'));
    if (!credentials?.baseUrl) {
        throw new n8n_workflow_1.NodeOperationError(this.getNode(), 'Base URL não configurada nas credenciais.');
    }
    ...
    try {
        ...
        const statusCode = extractHttpStatusCode(response);
        const headers = isRecord(response) && isRecord(response.headers)
            ? response.headers
            : {};
        ...
        for (let i = 0; i < items.length; i++) {
            try {
                ...
                if (operation === 'getById') {
                    ...
                    const getByIdOptions = this.getNodeParameter('getByIdOptions', i, {});
                    const downloadAttachmentFile = entity === 'Attachment' && getByIdOptions.downloadAttachmentFile === true;
                ...
                if (operation === 'create') {
                    ...
                    const payload = (() => {
                        if (inputMode === 'json')
                            return getJsonObjectParameter(this, i, 'createPayloadJson');
                        if (isToolNode(this)) {
                            const toolPayload = getFieldAssignments(this, i, 'createFieldsTool');
                            if (Object.keys(toolPayload).length > 0)
                                return toolPayload;
                            const legacyAiPayload = getAssignmentCollectionPayload(this, i, 'createFieldsAi');
                            if (Object.keys(legacyAiPayload).length > 0)
                                return legacyAiPayload;
                ...
                if (operation === 'update') {
                    ...
                    const payload = (() => {
                        if (inputMode === 'json')
                            return getJsonObjectParameter(this, i, 'updatePayloadJson');
                        if (isToolNode(this)) {
